...
namespace famsdatautil {
            ...
            Poco::Net::HTTPServerResponse &response)
    {
		...
		else if (mRequestType == RequestType::LOGOUT_POST)
		{
	...
	void FAMSDUUserHandler::handleLogoutRequest(Poco::Net::HTTPServerRequest& request,
												Poco::Net::HTTPServerResponse& response)
	{
		 mdb_apl_log_start();
    ...
    void FAMSDUUserHandler::handleGetUsersRequest(
        ...
        if (mdbcom::Result::OK == retExec) {
            ...
            for (int i = 0; i < selectStm->getNumRows(); i++) {
                ...
                selectStm->getValue(i, selectStm->getColumnIndex("userId"), userId);
				selectStm->getValue(i, selectStm->getColumnIndex("name"), name);
				selectStm->getValue(i, selectStm->getColumnIndex("role"), role);
                selectStm->getValue(i, selectStm->getColumnIndex("mail"), mail);
                selectStm->getValue(i, selectStm->getColumnIndex("activities"), activities);
                userModel.setId(id);
                userModel.setUserId(userId);
				userModel.setName(name);
				userModel.setRole(role);
                userModel.setMail(mail);
                userModel.setActivities(activities);
            ...
            const std::vector<FAMSDUUserData> &userStore)
    {
        ...
        if (OK == retCode)
        {
            ...
            for(auto userData: userStore) {
                ...
                jsonEncoder.setKeyMapDataValue(userTag, USER_ID, userData.getUserId());
                jsonEncoder.setKeyMapDataValue(userTag, NAME, userData.getName());
				jsonEncoder.setKeyMapDataValue(userTag, ROLE, userData.getRole());
                jsonEncoder.setKeyMapDataValue(userTag, MAIL, userData.getMail());
				jsonEncoder.setKeyMapDataValue(userTag, ACTIVITIES, userData.getActivities());
