...
namespace famsdatareception {
            ...
            std::size_t &dataLength)
    {
        ...
        extendedSize += mRequestData.getTimeStamp().size() * famscommon::TIME_STAMP_LEN;
        extendedSize += mRequestData.getRtuId().size()  * sizeof(int);
        extendedSize += mRequestData.getRtuChannel().size() * sizeof(int);
        ...
        catch (const std::bad_alloc &ex)
        {
            ...
            mdb_apl_log_critical("Can't allocate memory for obstalce information buffer [%s]", exDetail);
        }
