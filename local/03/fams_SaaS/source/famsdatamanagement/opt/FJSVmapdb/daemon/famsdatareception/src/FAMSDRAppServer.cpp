...
namespace famsdatareception {
    ...
    bool FAMSDRAppServer::initializeApplication(void)
    {
        ...
        try
        {
            ...
            // Init FAMSDRRequestHandlerFactory
            auto queue = mSharedQueue.get();
            runningPosition = std::string {"creating FAMSDRRequestHandlerFactory"};
            mHandlerFactory = mdbcom::MDBMemoryUtility::make_unique <FAMSDRRequestHandlerFactory>(
