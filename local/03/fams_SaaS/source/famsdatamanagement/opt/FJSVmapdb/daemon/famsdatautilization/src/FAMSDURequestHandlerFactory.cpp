...
namespace famsdatautil {
    ...
    std::map<std::string, std::map<std::string, std::vector<mdbcom::MDBContentDecodingFormat>>> FAMSDURequestHandlerFactory::requestTemplates {
		...
		{URI_LOGOUT_PATH, {{Poco::Net::HTTPRequest::HTTP_POST, std::vector<mdbcom::MDBContentDecodingFormat>()}}},
        ...
        const Poco::Net::HTTPServerRequest &request)
    {
        ...
        do
        {
            ...
            try {
				...
				else if (Poco::RegularExpression(famscommon::replaceParentheses(URI_LOGOUT_PATH)).match(uriPath))
				{
					auto urlDetail = requestTemplates.find(URI_LOGOUT_PATH)->second;
					if (urlDetail.find(request.getMethod()) == urlDetail.end()) {
						mdb_apl_log_error("Method " + request.getMethod() + " isn't supported");
						// TODO: Return error
						break;
					}
					handler = new FAMSDUUserHandler(urlDetail.find(request.getMethod())->second, RequestType::LOGOUT_POST);
				}
