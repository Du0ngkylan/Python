...
/*!
 * @file        FAMSEPWaterReplaceHandler.cpp
...
namespace famseventprocessing {
     ...
     * @return None.
     */
    FAMSEPWaterReplaceHandler::FAMSEPWaterReplaceHandler(
     ...
     */
    int FAMSEPWaterReplaceHandler::handleEvent(void)
    {
        mdb_apl_log_start();
        double startTime = famscommon::getCurrentTime();
        std::size_t size;
        if (famscommon::MessageType::SENSOR_DATA != mShrMessage->ipcType)
        {
        ...
        // Received data
        auto recData = reinterpret_cast<const famscommon::FAMSIpcWaterReplaceData *>(mShrMessage); /* pgr0745 */
        const double *water_level = recData->getWaterLevel(size);
        ...
        for(std::vector<double>::size_type i = 0; i != size; i++) {
            ...
            params.emplace_back(water_level[i]);
            params.emplace_back(water_temp[i]);
            auto selectStm = dbConnection.execute(INSERT_REPLACE_TANK_DATA, params); /*pgr0745*/
            params.clear();
