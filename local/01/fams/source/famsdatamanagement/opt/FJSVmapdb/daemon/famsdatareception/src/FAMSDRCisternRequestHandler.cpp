...
/*!
 * @file        FAMSDRCisternRequestHandler.cpp
...
namespace famsdatareception {
     ...
     * @return None.
     */
    FAMSDRCisternRequestHandler::FAMSDRCisternRequestHandler(mdbcom::MDBIPCShm *sharedMemory,
     ...
     */
    int FAMSDRCisternRequestHandler::validateJSONFormat(
        ...
        std::vector<std::string> &errKeys)
    {
        ...
        auto inputAnalyzer = FAMSDRCisternRequestAnalyzer {&jsonDecoder};
    ...
    FAMSDRCisternRequestHandler::getIpcInfoData(const famscommon::MessageType eventType,
            std::size_t &dataLength)
    {
        ...
        extendedSize += mRequestData.getInFlowTemp().size() * sizeof(double);
        extendedSize += mRequestData.getOutFlowTemp().size() * sizeof(double);
        extendedSize += mRequestData.getUpperCentralIll().size() * sizeof(int);
        extendedSize += mRequestData.getLowerCentralIll().size() * sizeof(int);
        extendedSize += mRequestData.getSalt().size() * sizeof(double);
        extendedSize += mRequestData.getPH().size() * sizeof(double);
        auto ret = std::unique_ptr<famscommon::FAMSIpcHeader> {};
        try
        {
            auto requestInformation = new (extendedSize) famscommon::FAMSIpcCisternSensorData {eventType, mRequestData};
            ret = std::unique_ptr<famscommon::FAMSIpcHeader>
            {
            	reinterpret_cast<famscommon::FAMSIpcHeader *>(requestInformation)
            };
            dataLength = sizeof(famscommon::FAMSIpcCisternSensorData) + extendedSize;
     ...
     */
    int FAMSDRCisternRequestHandler::registerEvent(void)
    {
        mdb_apl_log_start();

        sendIpcInfoData(famscommon::MessageType::CISTERN_SENSOR_DATA);
