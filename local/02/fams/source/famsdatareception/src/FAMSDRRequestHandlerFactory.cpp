...
//    const Poco::RegularExpression FAMSDRWaterTankRequestHandlerFactory::URI_SENSOR(URI_SENSOR_PATH);
    ...
    FAMSDRWaterTankRequestHandlerFactory::FAMSDRWaterTankRequestHandlerFactory(mdbcom::MDBIPCShm *sharedMemory,
        ...
        mSharedMemory { sharedMemory }, mReplaceTTemplate { }, mNitrificationTemplate {}, mOutSideTemplate{}, mCisternAddTemplate{}
    {
		...
		if (mdbcom::Result::OK != templateConf.getFormat(CISTERN_ADD_PATH, Poco::toLower(Poco::Net::HTTPRequest::HTTP_POST), mCisternAddTemplate))
		{
        ...
        do
        {
            if (request.getMethod() == Poco::Net::HTTPRequest::HTTP_POST)
            {
                try
                {
					...
					else if (uriPath.compare(CISTERN_ADD_PATH) == 0) {
						handler = new FAMSDRSensorRequestHandler(mSharedMemory, mCisternAddTemplate);
						break;
					}
                    else
                    {
