...
/*!
 * @file        FAMSDRWaterTankRequestHandler.cpp
...
namespace famsdatareception {
     ...
     * @return None.
     */
    FAMSDRWaterTankRequestHandler::FAMSDRWaterTankRequestHandler(mdbcom::MDBIPCShm *sharedMemory,
     ...
     */
    int FAMSDRWaterTankRequestHandler::validateJSONFormat(
        ...
        std::vector<std::string> &errKeys)
    {
        ...
        auto inputAnalyzer = FAMSDRRqWaterReplaceAnalyzer {&jsonDecoder};
    ...
    FAMSDRWaterTankRequestHandler::getIpcInfoData(const famscommon::MessageType eventType,
            std::size_t &dataLength)
    {
        mdb_apl_log_start();
        auto extendedSize = 0ull;
        extendedSize += mRequestData.getWaterLevel().size() * sizeof(double);
        ...
        try
        {
            auto requestInformation = new (extendedSize) famscommon::FAMSIpcWaterReplaceData {eventType, mRequestData};
            ret = std::unique_ptr<famscommon::FAMSIpcHeader>
            {
            	reinterpret_cast<famscommon::FAMSIpcHeader *>(requestInformation)
            };
            dataLength = sizeof(famscommon::FAMSIpcWaterReplaceData) + extendedSize;
     ...
     */
    int FAMSDRWaterTankRequestHandler::registerEvent(void)
    {
        mdb_apl_log_start();

        sendIpcInfoData(famscommon::MessageType::SENSOR_DATA);
