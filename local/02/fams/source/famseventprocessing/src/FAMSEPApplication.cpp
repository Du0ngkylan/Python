...
namespace famseventprocessing {
    ...
    const std::string FAMSEPApplication::mSensorAddShmName { "Processing_Sensor_Add" };
    ...
    { nullptr };
    std::unique_ptr<mdbcom::MDBIPCShm> FAMSEPApplication::mSensorAddShareQueue
    { nullptr };
    /*!
     * @brief Constructor
     * @param None.
            ...
            const void *arg __attribute__((unused)))
    {
        ...
        if (nullptr != mSensorAddShareQueue)
        {
            (mSensorAddShareQueue.get())->endRead();
        }
    ...
    void FAMSEPApplication::eventHandler(void *args __attribute__((unused)))   /* pgr0304 */
    {
        ...
        while (true)
        {
            ...
            else if (famscommon::MessageType::SENSOR_ADD_DATA == message->ipcType)
            {
                mdb_apl_log_trace("Forward Sensor Add SENSOR message ");
                auto messagePtr = message.get();/*pgr0745*/
                // sensor queue: FAMSIpcHeader part
                writeRetCode = mSensorAddShareQueue->writeData(messagePtr, messageSize); // forward = false, data size = real size of data
            }
            else
            {
   ...
   /*!
     ...
     * @return None.
     */
    void FAMSEPApplication::sensorAddHandler(void *args)
    {
        mdb_apl_log_start();
        static_cast<void>(args);
        auto handler = FAMSEPSensorHandler {};/*pgr0745*/

        while (true)
        {
            auto messageSize = std::size_t { 0 };/*pgr0745*/
            auto message = mSensorAddShareQueue->readData<famscommon::FAMSIpcHeader>(messageSize); /* pgr0745 */
            // Reading failed abnormally
            if (nullptr == message)
            {
                // detect system end
    ...
    bool FAMSEPApplication::initializeApplication(void)
    {
        ...
        try
        {
            ...
            runningPosition = std::string{"initializing SensorAddSharedQueue shared queue"};
            mSensorAddShareQueue = mdbcom::MDBMemoryUtility::make_unique <
                                           mdbcom::MDBIPCShm > (mSensorAddShmName,
                                                   eventProcessingConfig.getThreadsShmSize(SENSOR_ADD_THREAD_NAME));
        }
    ...
    bool FAMSEPApplication::uninitializeApplication(void)
    {
        ...
        try
        {
            ...
            if (nullptr != mSensorAddShareQueue)
            {
                (mSensorAddShareQueue.get())->unlink();
            }
            ...
            std::vector<mdbcom::MDBThreadDatabaseConfig> &dbConfig, mdbcom::ThreadFunction &threadFunction)
    {
        ...
        else if (SENSOR_ADD_THREAD_NAME == threadName)
        {
            commonStatements.insert(commonStatementsEndIt, sensorAddPreparedStatements.begin(), sensorAddPreparedStatements.end());
            dbConfig.emplace_back(commonStatements, mdbcom::ConnectionType::DYNAMIC);
            threadFunction = sensorAddHandler;
        }
        else
        {
            dbConfig.clear();
    ...
    int FAMSEPApplication::execute(const std::vector<std::string> &args __attribute__((unused)))
    {
        ...
        else
        {
         ...
         else
         {
          ...
          //! Init sensor add Threads
          auto numberOfSensorAddThreads = eventProcessingConfig.getThreadsNum(SENSOR_ADD_THREAD_NAME); /* pgr0745 */
          std::promise<bool> sensorAddPromise[numberOfSensorAddThreads];
          std::future<bool> sensorAddFuture[numberOfSensorAddThreads];
          std::thread sensorAddThreads[numberOfSensorAddThreads];
          auto sensorAddDBConfig = std::vector<mdbcom::MDBThreadDatabaseConfig> { }; /* pgr0745 */
		  std::vector<mdbcom::MDBPreparedStatement> sensorAddStatements;
          mdb_apl_log_debug("starting all obstacle detection thread...");

           if (false == isThreadStartFailed)
           {
			   // Get SQLstatement from the config file
			   const std::set<int> listPorts = eventProcessingConfig.getPortTypes();
			   for (auto i = listPorts.cbegin(); i != listPorts.cend(); ++i)
			   {
				   sensorAddStatements.push_back(sensorAddPreparedStatements[*i - 1]);
			   }
			   sensorAddStatements.insert(sensorAddStatements.end(), cisternInformationsPreparedStatements.begin(), cisternInformationsPreparedStatements.end());
               sensorAddStatements.insert(sensorAddStatements.end(), commonStatementsBeginIt, commonStatementsEndIt);
			   sensorAddStatements.insert(sensorAddStatements.end(), cisternSensorPreparedStatements.begin(), cisternSensorPreparedStatements.end());
               sensorAddDBConfig.emplace_back(sensorAddStatements, mdbcom::ConnectionType::DYNAMIC);

              // Check if the threads executed.
              for (auto i = 0u; i < numberOfSensorAddThreads; i++) /* pgr0745 *//* pgr0692 */
              {
                  sensorAddFuture[i] = sensorAddPromise[i].get_future();

                  if (mdbcom::Result::OK != mdbcom::MDBThreadUtility::createDBWorkUnit(SENSOR_ADD_THREAD_NAME,
                          sensorAddHandler, sensorAddDBConfig, &sensorAddPromise[i], sensorAddThreads[i]))
                  {
                      mdb_apl_log_critical("Can't create obstacle detection thread #%u", i);
                      isThreadStartFailed = true;
                      sensorAddPromise[i].set_value(true);

                      // Set promise value to prevent waiting for a non-executing thread
                      for (auto j = i + 1; j < numberOfSensorAddThreads; j++)
                      {
                          sensorAddFuture[j] = sensorAddPromise[j].get_future();
                          sensorAddPromise[j].set_value(true);
                      }
        ...
        for (auto j = 0u; j < numberOfSensorAddThreads; j++) /* pgr0745 *//* pgr0692 */
        {
            if (false == sensorAddThreads[j].joinable())
            {
                mdb_apl_log_warn("obstacleDetectionThreads [%u] is unjoinable", j);
            }
            else
            {
                try
                {
                    sensorAddThreads[j].join();
                    mdb_apl_log_info("obstacleDetectionThreads [%u] ended", j);
                }
                catch (...)
                {
                    mdb_apl_log_warn("Error waiting for sensorAddThreads [%u]", j);
                }
            }
        }
        mdb_apl_log_end();
