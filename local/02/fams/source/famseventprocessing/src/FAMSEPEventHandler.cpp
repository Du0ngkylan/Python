...
namespace famseventprocessing {
    ...
    int FAMSEPEventHandler::checkSensorValue(const double value, const int type, const std::string cistern_code,
        ...
        if (mdbcom::Result::OK == retExec) {
            ...
            changeStatusResolved(type, RESOLVED_ERROR_ST, cistern_code);
            ...
            for (int i=0; i < numRows; i++) {
                ...
                {
                    int rtTemp = snprintf(buffer, 10000, "塩分濃度: %s (%s)", violation.c_str(), organismName.c_str());
                }
                else if(type == WATER_TMP_IN_ERR)
                {
                    int rtTemp = snprintf(buffer, 10000, "水温: %s (%s)", violation.c_str(), organismName.c_str());
                }
				else if(type == WATER_TMP_OUT_ERR)
                {
                    int rtTemp = snprintf(buffer, 10000, "濁度: %s (%s)", violation.c_str(), organismName.c_str());
                }
                 else if(type == WATER_TMP_DISSOL_ERR)
                {
                    int rtTemp = snprintf(buffer, 10000, "溶存酸素: %s (%s)", violation.c_str(), organismName.c_str());
                }
                else
                {
                    //do nothing
    ...
    bool FAMSEPEventHandler::changeStatusResolved(const int type, const int status, const std::string cistern_code)
    {
        mdb_apl_log_start();
        bool ret = true;
        ...
        params.emplace_back(cistern_code);
        const auto &dbConnection = mdbcom::MDBConnectionManager::getDynamicDBConnection();
