...
namespace famsdatautil {
            ...
            Poco::Net::HTTPServerResponse &response)
    {
		...
		else if (mRequestType == RequestType::ALL_TASKS_GET) 
		{
			handlerGetAllTasksRequest(request, response);
		}
            ...
            else
            {
                ...
                if (taskType == 1) {
                    ...
                    if (mdbcom::Result::OK == retExec) {
                        ...
                        selectStm->getValue(0, selectStm->getColumnIndex("memo"), returnedMemo);
                        monitorInputData.setProcessTime(returnedProcessTime);
	...
	void FAMSDUTaskHandler::handlerGetAllTasksRequest(Poco::Net::HTTPServerRequest& request,
		Poco::Net::HTTPServerResponse& response)
	{
		mdb_apl_log_start();
		// Declare variable
		...
		auto jsonResponse = std::string{};/*pgr0745*/
		const auto& dbConnection = mdbcom::MDBConnectionManager::getDynamicDBConnection();
		auto selectInforStm = dbConnection.execute(SELECT_TASKS_INFOR_QUERY); /*pgr0745*/
		auto retExec = selectInforStm->getResult();
		int taskType;
		long long taskId;
		auto params = std::vector<mdbcom::MDBDatabaseParam>{}; /*pgr0745*/
		std::vector<mdbcom::MDBContentParser::KeyMapType> pondInputDatas;
		std::vector<mdbcom::MDBContentParser::KeyMapType> feedingInputDatas;
		std::vector<mdbcom::MDBContentParser::KeyMapType> recoveryInputDatas;
		std::vector<mdbcom::MDBContentParser::KeyMapType> disposalInputDatas;
		std::vector<mdbcom::MDBContentParser::KeyMapType> shippingInputDatas;
		std::vector<mdbcom::MDBContentParser::KeyMapType> waterInputDatas;
		std::vector<mdbcom::MDBContentParser::KeyMapType> maintenanceInputDatas;
		std::vector<mdbcom::MDBContentParser::KeyMapType> monitorInputDatas;
		mdbcom::MDBJsonParser jsonEncoderRes{};
		jsonEncoderRes.setSerialKey(0);
		auto jsonMapRes = mdbcom::MDBContentParser::KeyMapType{};/*pgr0745*/
		if (mdbcom::Result::OK == retExec)
		{
			for (int i = 0; i < selectInforStm->getNumRows(); i++)
			{
				selectInforStm->getValue(i, selectInforStm->getColumnIndex("id"), taskId);
				selectInforStm->getValue(i, selectInforStm->getColumnIndex("type"), taskType);
				mdb_apl_log_debug("==============Task taskId=%d", taskId);
				mdb_apl_log_debug("==============Task Type=%d", taskType);
				params.emplace_back(taskId);
				if (taskType == 1) {
					auto selectStm = dbConnection.execute(SELECT_TASK_POND_INPUT_QUERY, params); /*pgr0745*/
					retExec = selectStm->getResult();
					FAMSDUTaskPondInputData pondInputData;
					if (mdbcom::Result::OK == retExec) {
						if (selectStm->getNumRows() > 0)
					...
					if (0 == errKeys.size())
					{
						...
						jsonEncoder.setKeyMapDataValue(jsonMap, ID, taskId);
						jsonEncoder.setKeyMapDataValue(jsonMap, TYPE, taskType);
						jsonEncoder.setKeyMapDataValue(jsonMap, POND_INPUT, inputTag);
					}
					else {
						auto errorsTag = buildErrorMessages(errKeys);
						if (0 != errorsTag.size())
					...
					pondInputDatas.emplace_back(jsonMap);
				}
				else if (taskType == 2) {
					auto selectStm = dbConnection.execute(SELECT_TASK_FEEDING_INPUT_QUERY, params); /*pgr0745*/
					retExec = selectStm->getResult();
					FAMSDUTaskFeedingInputData feedingInputData;
					if (mdbcom::Result::OK == retExec) {
						std::string returnedProcessTime;
					...
					if (0 == errKeys.size())
					{
						...
						jsonEncoder.setKeyMapDataValue(jsonMap, ID, taskId);
						jsonEncoder.setKeyMapDataValue(jsonMap, TYPE, taskType);
						jsonEncoder.setKeyMapDataValue(jsonMap, FEEDING_INPUT, inputTag);
					}
					else {
						auto errorsTag = buildErrorMessages(errKeys);
						if (0 != errorsTag.size())
					...
					feedingInputDatas.emplace_back(jsonMap);
				}
				else if (taskType == 3) {
					auto selectStm = dbConnection.execute(SELECT_TASK_RECOVERY_INPUT_QUERY, params); /*pgr0745*/
					retExec = selectStm->getResult();
					FAMSDUTaskRecoveryInputData recoveryInputData;
					if (mdbcom::Result::OK == retExec) {
						std::string returnedProcessTime;
					...
					if (0 == errKeys.size())
					{
						...
						jsonEncoder.setKeyMapDataValue(jsonMap, ID, taskId);
						jsonEncoder.setKeyMapDataValue(jsonMap, TYPE, taskType);
						jsonEncoder.setKeyMapDataValue(jsonMap, RECOVERY_INPUT, inputTag);
					}
					else {
						auto errorsTag = buildErrorMessages(errKeys);
						if (0 != errorsTag.size())
					...
					recoveryInputDatas.emplace_back(jsonMap);
				}
				else if (taskType == 4) {
					auto selectStm = dbConnection.execute(SELECT_TASK_DISPOSAL_INPUT_QUERY, params); /*pgr0745*/
					retExec = selectStm->getResult();
					FAMSDUTaskDisposalInputData disposalInputData;
					if (mdbcom::Result::OK == retExec) {
						std::string returnedProcessTime;
					...
					if (0 == errKeys.size())
					{
						...
						jsonEncoder.setKeyMapDataValue(jsonMap, ID, taskId);
						jsonEncoder.setKeyMapDataValue(jsonMap, TYPE, taskType);
						jsonEncoder.setKeyMapDataValue(jsonMap, DISPOSAL_INPUT, inputTag);
					}
					else {
						auto errorsTag = buildErrorMessages(errKeys);
						if (0 != errorsTag.size())
					...
					disposalInputDatas.emplace_back(jsonMap);
				}
				else if (taskType == 5) {
					auto selectStm = dbConnection.execute(SELECT_TASK_SHIPPING_INPUT_QUERY, params); /*pgr0745*/
					retExec = selectStm->getResult();
					FAMSDUTaskShippingInputData shippingInputData;
					if (mdbcom::Result::OK == retExec) {
						std::string returnedProcessTime;
					...
					if (0 == errKeys.size())
					{
						...
						jsonEncoder.setKeyMapDataValue(jsonMap, ID, taskId);
						jsonEncoder.setKeyMapDataValue(jsonMap, TYPE, taskType);
						jsonEncoder.setKeyMapDataValue(jsonMap, SHIPPING_INPUT, inputTag);
					}
					else {
						auto errorsTag = buildErrorMessages(errKeys);
						if (0 != errorsTag.size())
					...
					shippingInputDatas.emplace_back(jsonMap);
				}
				else if (taskType == 6) {
					auto selectStm = dbConnection.execute(SELECT_TASK_WATER_INPUT_QUERY, params); /*pgr0745*/
					retExec = selectStm->getResult();
					FAMSDUTaskWaterInputData waterInputData;
					if (mdbcom::Result::OK == retExec) {
						std::string returnedProcessTime;
					...
					if (0 == errKeys.size())
					{
						...
						jsonEncoder.setKeyMapDataValue(jsonMap, ID, taskId);
						jsonEncoder.setKeyMapDataValue(jsonMap, TYPE, taskType);
						jsonEncoder.setKeyMapDataValue(jsonMap, WATER_INPUT, inputTag);
					}
					else {
						auto errorsTag = buildErrorMessages(errKeys);
						if (0 != errorsTag.size())
					...
					waterInputDatas.emplace_back(jsonMap);
				}
				else if (taskType == 7) {
					auto selectStm = dbConnection.execute(SELECT_TASK_MAINTENANCE_INPUT_QUERY, params); /*pgr0745*/
					retExec = selectStm->getResult();
					FAMSDUTaskMaintenanceInputData maintenanceInputData;
					if (mdbcom::Result::OK == retExec) {
						std::string returnedProcessTime;
					...
					if (0 == errKeys.size())
					{
						...
						jsonEncoder.setKeyMapDataValue(jsonMap, ID, taskId);
						jsonEncoder.setKeyMapDataValue(jsonMap, TYPE, taskType);
						jsonEncoder.setKeyMapDataValue(jsonMap, MAINTENANCE_INPUT, inputTag);
					}
					else {
						auto errorsTag = buildErrorMessages(errKeys);
						if (0 != errorsTag.size())
					...
					maintenanceInputDatas.emplace_back(jsonMap);
				}
				else if (taskType == 8) {
				    auto selectStm = dbConnection.execute(SELECT_TASK_WATER_MONITOR_INPUT_QUERY, params); /*pgr0745*/
					retExec = selectStm->getResult();
					FAMSDUTaskMonitorInputData monitorInputData;
					if (mdbcom::Result::OK == retExec) {
						std::string returnedProcessTime;
					...
					if (0 == errKeys.size())
					{
						...
						jsonEncoder.setKeyMapDataValue(jsonMap, ID, taskId);
						jsonEncoder.setKeyMapDataValue(jsonMap, TYPE, taskType);
						jsonEncoder.setKeyMapDataValue(jsonMap, WATER_MONITOR_INPUT, inputTag);
					}
					else {
						auto errorsTag = buildErrorMessages(errKeys);
						if (0 != errorsTag.size())
					...
					monitorInputDatas.emplace_back(jsonMap);
				}
				httpRetCode = Poco::Net::HTTPResponse::HTTP_OK;
				params.clear();
			}
			std::vector<mdbcom::MDBContentParser::KeyMapType> allTasksData;
			allTasksData.insert(allTasksData.end(), pondInputDatas.begin(), pondInputDatas.end());
			allTasksData.insert(allTasksData.end(), feedingInputDatas.begin(), feedingInputDatas.end());
			allTasksData.insert(allTasksData.end(), recoveryInputDatas.begin(), recoveryInputDatas.end());
			allTasksData.insert(allTasksData.end(), disposalInputDatas.begin(), disposalInputDatas.end());
			allTasksData.insert(allTasksData.end(), shippingInputDatas.begin(), shippingInputDatas.end());
			allTasksData.insert(allTasksData.end(), waterInputDatas.begin(), waterInputDatas.end());
			allTasksData.insert(allTasksData.end(), maintenanceInputDatas.begin(), maintenanceInputDatas.end());
			allTasksData.insert(allTasksData.end(), monitorInputDatas.begin(), monitorInputDatas.end());
			jsonEncoderRes.setKeyMapDataArrayValue(jsonMapRes, DATA, allTasksData);
		}
		 else 
		{
			 auto errorsTag = buildErrorMessages(errKeys);
			 if (0 != errorsTag.size())
			 {
				 jsonEncoderRes.setKeyMapDataArrayValue(jsonMapRes, ERRORS, errorsTag);
			 }
		}

		auto ret = jsonEncoderRes.encodeKeyMapData(jsonMapRes, jsonResponse);/*pgr0745*/
        ...
        do {
            ...
            if (mdbcom::Result::OK == retExec) {
                if (selectStm->getNumRows() > 0) {
					...
					selectStm = dbConnection.execute(SELECT_ERROR_MESSAGES, errorParam);
                    //update status = 2 for all lot which are no longer violated
                    auto numDelMsgRecordeds = changeStatusResolved(convertMonitorType, cisternId);
                }
            }

            //check violation then import to error_message
            auto numViolations = checkSensorValue(stime, monitorValue, convertMonitorType, cisternId, statusOfThisRecord);

			if (statusOfThisRecord == OCCURRING_ERRMSG)
			{
				// get cistern name
				auto selectNameStm = dbConnection.execute(SELECT_CISTERN_NAME, errorParam);
				std::string cisternName;
				if (mdbcom::Result::OK == selectNameStm->getResult())
				{
					if (selectNameStm->getNumRows() > 0)
					{
						selectNameStm->getValue(0, selectNameStm->getColumnIndex("name"), cisternName);
					}
				}
				...
				if (mdbcom::Result::OK == selectErrorStm->getResult())
				{
					...
					auto paramGetValues = std::vector<mdbcom::MDBDatabaseParam>{};
					...
					if (mdbcom::Result::OK == selectStm->getResult())
					{
						...
						for (int i = 0; i < selectStm->getNumRows(); i++)
						{
							selectStm->getValue(i, selectStm->getColumnIndex("resource_id"), resourceId);
							selectStm->getValue(i, selectStm->getColumnIndex("resource_kind"), resourceKind);
							selectStm->getValue(i, selectStm->getColumnIndex("error_at"), errorAt);
							selectStm->getValue(i, selectStm->getColumnIndex("error_message"), errorMessage);
							resourceIds.emplace_back(resourceId);
							resourceKinds.emplace_back(resourceKind);
							errorAts.emplace_back(errorAt);
							errorMessages.emplace_back(errorMessage);
							paramGetValues.emplace_back(cisternId);
							paramGetValues.emplace_back(errorAt - 3);
							auto selectMinotorInputStm = dbConnection.execute(SELECT_TASK_MONITOR_INPUT, paramGetValues);
							if (mdbcom::Result::OK == selectMinotorInputStm->getResult())
							{
								if (selectMinotorInputStm->getNumRows() > 0)
								{
									selectMinotorInputStm->getValue(0, selectMinotorInputStm->getColumnIndex("monitor_value"), value);
								}
							}
							monitorValues.emplace_back(value);
							paramGetValues.clear();
						...
						{
							if (std::find(resourceIds.begin(), resourceIds.end(), resourceIdAdds.at(n)) == resourceIds.end() ||
							    std::find(resourceKinds.begin(), resourceKinds.end(), resourceKindAdds.at(n)) == resourceKinds.end() ||
							    std::find(errorAts.begin(), errorAts.end(), errorAtAdds.at(n)) == errorAts.end())
						...
						for (int m = 0; m < selectStm->getNumRows(); m++)
						{
							if (std::find(resourceIdAdds.begin(), resourceIdAdds.end(), resourceIds.at(m)) == resourceIdAdds.end() ||
								std::find(resourceKindAdds.begin(), resourceKindAdds.end(), resourceKinds.at(m)) == resourceKindAdds.end() ||
								std::find(errorAtAdds.begin(), errorAtAdds.end(), errorAts.at(m)) == errorAtAdds.end())
								mdb_apl_log_mes_info("Recovery error_message: %s, %s, %.3f", errorMessages.at(m).c_str(), cisternName.c_str(), monitorValues.at(m));
						}
