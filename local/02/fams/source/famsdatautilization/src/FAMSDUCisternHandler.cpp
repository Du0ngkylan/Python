                ...
                for (int iCistern=0; iCistern< selectStm->getNumRows();iCistern++) {
                    ...
                    if (mdbcom::Result::OK == retExecMHistory) {
                        ...
                        for (int iHistory=0; iHistory< selectMHistory->getNumRows();iHistory++) {
							...
							std::string monitorDate_add_sensor, ictMonitorDate_add_sensor;
                            std::string monitorInFlowTemp;
                            std::string monitorTurbidity;
                            std::string monitorSalt;
                            std::string monitorPH;
							std::string monitorDissolvedOxygen;
                            selectMHistory->getValue(iHistory, selectMHistory->getColumnIndex("accumulated_time_rs"), ictMonitorDate);
							selectMHistory->getValue(iHistory, selectMHistory->getColumnIndex("accumulated_time_add_sensor_rs"), ictMonitorDate_add_sensor);
							...
							selectMHistory->getValue(iHistory, selectMHistory->getColumnIndex("dissolved_oxygen_rs"), monitorDissolvedOxygen);
							...
							ret = famscommon::convertICTToUTC(ictMonitorDate_add_sensor, monitorDate_add_sensor);
							if (ret == mdbcom::Result::OK) {
								historyList.find(SensorType::DISSOLVED_OXYGEN)->second.insert(std::pair<std::string, std::string>(monitorDate_add_sensor, monitorDissolvedOxygen));
							}
							else {
								historyList.find(SensorType::DISSOLVED_OXYGEN)->second.insert(std::pair<std::string, std::string>(ictMonitorDate_add_sensor, monitorDissolvedOxygen));
							}
                        }
                    }
                    ...
                    if (mdbcom::Result::OK == retExecIon) {
                        ...
                        for (int iHistory=0; iHistory< selectIon->getNumRows();iHistory++) {
                            std::string ammoniumDate, ictAmmoniumDate;
                            std::string nitriteDate, ictNitriteDate;
                            std::string nitrateDate, ictNitrateDate;
                            ...
                            auto ret1 = famscommon::convertICTToUTC(ictAmmoniumDate, ammoniumDate);
                            auto ret2 = famscommon::convertICTToUTC(ictNitriteDate, nitriteDate);
                            auto ret3 = famscommon::convertICTToUTC(ictNitrateDate, nitrateDate);
                    ...
                    if (mdbcom::Result::OK == retExecMonitor) {
                        ...
                        for (int iMonitor=0; iMonitor< selectMonitor->getNumRows();iMonitor++) {
							...
							std::string monitorAccumulatedDate_add_sensor, ictAccumulatedDate_add_sensor;
                            std::string monitorDate, ictDate;
                            std::string monitorInFlowTemp;
							...
							selectMonitor->getValue(iMonitor, selectMonitor->getColumnIndex("accumulated_time_add_sensor"), ictAccumulatedDate_add_sensor);
                            ...
                            selectMonitor->getValue(iMonitor, selectMonitor->getColumnIndex("dissolved_oxygen"), monitorDissolvedOxygen);
							...
							monitorParams.emplace_back(ictAccumulatedDate_add_sensor);
                            ...
                            auto ret1 = famscommon::convertICTToUTC(ictDate, monitorDate);
                            auto ret2 = famscommon::convertICTToUTC(ictAccumulatedDate, monitorAccumulatedDate);
							auto ret3 = famscommon::convertICTToUTC(ictAccumulatedDate_add_sensor, monitorAccumulatedDate_add_sensor);
                            for (auto const& sensorType: sensorList) {
                                ...
                                if (mdbcom::Result::OK == retExecErrMsgs)
                                {
                                    for (int iErr=0; iErr< selectErrMsgs->getNumRows();iErr++) {
                                        ...
                                        case 6:
                                            ...
                                            break;
										case 7:
											errorAt = SensorType::DISSOLVED_OXYGEN;
											break;
                                        }
								...
								case SensorType::DISSOLVED_OXYGEN:
									monitorRecord.setValue(monitorDissolvedOxygen);
									monitorRecord.setDate(monitorAccumulatedDate_add_sensor);
									break;
					...
					// get tasks
					std::vector<FAMSDUTaskSummaryData> tasksList;
					auto selectTasks = dbConnection.execute(SELECT_TASKS_QUERY, params);
					auto retExeTasks = selectTasks->getResult();
					if (mdbcom::Result::OK == retExeTasks)
					{
						std::string processTime;
						int userID, taskID, type;
						std::string summary;
						...
						for (int iTask = 0; iTask < selectTasks->getNumRows(); iTask++)
						{
							selectTasks->getValue(iTask, selectTasks->getColumnIndex("id"), taskID);
							selectTasks->getValue(iTask, selectTasks->getColumnIndex("process_user_id"), userID);
							selectTasks->getValue(iTask, selectTasks->getColumnIndex("summary"), summary);
							selectTasks->getValue(iTask, selectTasks->getColumnIndex("process_time"), processTime);
							selectTasks->getValue(iTask, selectTasks->getColumnIndex("type"), type);
							FAMSDUTaskSummaryData taskData;
							taskData.setType(type);
							taskData.setId(taskID);
							std::string utcDate;
							auto ret = famscommon::convertICTToUTC(processTime, utcDate);
							if (ret == mdbcom::Result::OK) {
								taskData.setProcessTime(utcDate);
							}
							else {
								taskData.setProcessTime(processTime);
							}
							taskData.setPicUserId(userID);
							taskData.setSummary(summary);
							if (type == TaskType::SHIPPING) 
							{
								taskData.setSummary(shippingDestination.find(taskID)->second);
							}
							tasksList.emplace_back(taskData);
						}

					}
                    cisternRecord.setMonitorData(monitorList);
                    cisternRecord.setReferenceData(referenceList);
					cisternRecord.setTaskData(tasksList);
            ...
            const std::vector<FAMSDUCisternData> &cisternList)
    {
        ...
        if (OK == retCode)
        {
            ...
            for (auto const &cisternRecord : cisternList) {
				...
				// Add task tags
				std::vector<mdbcom::MDBContentParser::KeyMapType> taksList;
				for (auto const& taskRecord : cisternRecord.getTaskData())
				{
					auto taskRecordTag = mdbcom::MDBContentParser::KeyMapType{};
					jsonEncoder.setKeyMapDataValue(taskRecordTag, ID, taskRecord.getId());
					jsonEncoder.setKeyMapDataValue(taskRecordTag, TYPE, (long long)taskRecord.getType());
					jsonEncoder.setKeyMapDataValue(taskRecordTag, PROCESS_TIME, taskRecord.getProcessTime());
					jsonEncoder.setKeyMapDataValue(taskRecordTag, PIC_USER_ID, taskRecord.getPicUserId());
					jsonEncoder.setKeyMapDataArrayValue(taskRecordTag, SUMMARY, taskRecord.getSummary());
					taksList.emplace_back(taskRecordTag);
				}
				jsonEncoder.setKeyMapDataArrayValue(cisternRecordTag, TASKS, taksList);
